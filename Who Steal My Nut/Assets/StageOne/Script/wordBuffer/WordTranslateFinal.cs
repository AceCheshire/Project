...
public class WordTranslateFinal : MonoBehaviour
{
    /*Important Static*/
    ...
    void Start()
    {
        ...
        tilemapGoal = GameObject.Find("wordRendererFinal").GetComponent<Tilemap>();
    ...
    private Vector3Int RendPointer(Vector3Int lastRendPos, char letter)
    {
        ...
        int blankRate = 0;
        bool isModify = false;
        Vector3Int space = new (0, 0, 0);
        if (letter == '\n')
            return new Vector3Int(windowLeftTop.x, lastRendPos.y - 11, lastRendPos.z);
        // Special situation: \n
            ...
            for (int i = 0; i <= 7; i++)
            {
                ...
                if (blankRate == 8 && !isModify)
                {
                    ...
                    if (appliableXValue > 103)//Change Line
                    {
                        ...
                        appliableYValue = lastRendPos.y - 11;
