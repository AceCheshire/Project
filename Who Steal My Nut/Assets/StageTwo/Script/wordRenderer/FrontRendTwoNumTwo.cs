...
public class FrontRendTwoNumTwo : MonoBehaviour
{
    /*Important Static*/
    ...
    void Update()
    {
        ...
        if(wordBuffer.GetComponent<WordTranslateTwoNumTwo>().isWaitingFrontRend == true)
        {
            Flush();
            rendererStartPointer = new Vector3Int(0, 0, 0);
            Rend();
            wordBuffer.GetComponent<WordTranslateTwoNumTwo>().isWaitingFrontRend = false;
        }// Rend ( with request )
        if (GameObject.Find("StageTwoSortingOrderConfig").
                GetComponent<SortStageTwoObject>().isAlert == true)
        {
